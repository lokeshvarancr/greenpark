...
function CombinedVisualModal({ open, onClose, subject, test, students }: {
  open: boolean;
  onClose: () => void;
  subject: string;
  test: string;
...
const HighLowPerformers: React.FC = () => {
  ...
  const highList = useMemo(() => {
    ...
    if (perfTest) filtered = filtered.filter(s => s.subject && perfTest ? s.subject : true);
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-8">
      ...
      <CombinedVisualModal open={visualOpen} onClose={() => setVisualOpen(false)} subject={visualSubject} test={visualTest} students={visualStudents} />
    </div>
  );
};
